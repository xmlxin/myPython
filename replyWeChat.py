...
def get_gzh_text(msg):
	...
	#return msg['Text']
    #itchat.send(msg['Text'],toUserName = MyUserName)
    print("保存的username"+ MyUserName)
    itchat.send(msg['Text'], MyUserName)
...
python 微信智能聊天机器人流程
	...
	b.吧接收到的内容发送给小冰
	c.接收小冰返回的消息，转发到好友
	注意:普通开发者足够使用，未做压力测试，消息太多可能会串发消息，需要修改把MyUserName修改成队列
